...
class countAllEntries(Resource):
    ...
    def get(self):
        ...
        c.execute("SELECT Task.id, Task.name, COUNT(LearningEntry.task), Task.pillar as num FROM Task LEFT OUTER JOIN LearningEntry ON Task.id = LearningEntry.task GROUP BY Task.id") #quotes is SQL command/query. question mark defines placeholder, second part - give tuple 
